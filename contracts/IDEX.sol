...
contract Exchange {
  ...
  function adminWithdraw(address token, uint256 amount, address payable user, uint256 nonce, uint8 v, bytes32 r, bytes32 s, uint256 feeWithdrawal) public onlyAdmin {
    bytes32 hash = keccak256(abi.encodePacked(address(this), token, bytes32(amount), user, bytes32(nonce)));
  ...
  function trade(uint256[8] memory tradeValues, address[4] memory tradeAddresses, uint8[2] memory v, bytes32[4] memory rs) public onlyAdmin {
    ...
    bytes32 orderHash = keccak256(abi.encodePacked(address(this), tradeAddresses[0], bytes32(tradeValues[0]), tradeAddresses[1], bytes32(tradeValues[1]), bytes32(tradeValues[2]), bytes32(tradeValues[3]), tradeAddresses[2]));
    if (ecrecover(keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", orderHash)), v[0], rs[0], rs[1]) != tradeAddresses[2]) revert();
    bytes32 tradeHash = keccak256(abi.encodePacked(orderHash, bytes32(tradeValues[4]), tradeAddresses[3], bytes32(tradeValues[5]))); 
